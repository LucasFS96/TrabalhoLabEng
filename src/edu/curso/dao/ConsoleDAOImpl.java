...
public class ConsoleDAOImpl implements ConsoleDAO {
	...
	public void adicionar(Console c) {
		...
		try {
			...
			psmt.setDate(4,  new java.sql.Date(
					c.getDtLancamento().getTime()));	
	...
	public void alterar(Console c) throws SQLException {
		String sql = "UPDATE consoles SET nome = ?, preco = ?,  dtLancamento = ?, produtora = ?, versao = ? WHERE id = ?";
		PreparedStatement psmt = con.prepareStatement(sql);
		psmt.setLong(1, c.getId());
		psmt.setString(2, c.getNome());
		psmt.setDouble(3,c.getPreco());
	...
	public List<Console> pesquisarPorNome(String nome) {
		 ...
		 String sql ="SELECT id, nome, preco, dtLancamento,produtora, versao FROM consoles " +
		 "WHERE nome like ?";
		 try {
		 PreparedStatement psmt = con.prepareStatement(sql);
		 ...
		 c.setVersao(rs.getString("dtLancamento"));
		 consoles.add( c );
		 }
		 } catch (SQLException e) {
		 e.printStackTrace();
